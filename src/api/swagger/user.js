const userPaths = {
      ...
      parameters: [
        {
          ...
          required: false,
          description:
            'The ID of the user whose owned plugins are being requested. Defaults to the authenticated user if not provided or if the authenticated user is not an admin.',
        },
      ],
      responses: {
        200: {
  ...
  '/api/v1/users/author-agreement': {
    post: {
      ...
      description:
        "Updates the user's record to indicate whether they have accepted the author agreement. If `accept` is true, the agreement date is recorded. If false, the agreement status is reset.",
      requestBody: {
        required: true,
        content: {
          'application/json': {
            schema: {
              ...
              properties: {
                accept: {
                  type: 'boolean',
                  description:
                    'Indicates whether the user accepts or declines the author agreement. If true, the agreement date is recorded; if false, the status is reset.',
                },
              },
              required: ['accept'],
            },
